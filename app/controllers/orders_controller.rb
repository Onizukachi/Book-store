class OrdersController < ApplicationController
  ...
  private
    ...
    def order_params
      permitted_params = [:name, :address, :email, :pay_type]

      case params[:order][:pay_type]
      when "Credit card"
        permitted_params += [:credit_card_number, :expiration_date]
      when "Check"
        permitted_params += [:routing_number, :account_number]
      when "Purchase order"
        permitted_params << :po_number
      end
